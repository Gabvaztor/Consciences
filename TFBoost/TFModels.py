...
class TFModels():
    ...
    def convolution_model_image(self):
        ...
        x_reshape = tf.reshape(x, [-1, self.input_rows_numbers, self.input_columns_numbers,
        ...
        for epoch in range(self.epoch_numbers):
            for i in range(self.trains):
                ...
                if self.show_info:
                    ...
                    pt('y_conv_shape', y__conv.shape)
                    pt('index_buffer_data', index_buffer_data)
                    if self.show_images and i == 0:
                        show_image_from_tensor(x_batch_feed, self.input_rows_columns_array)  # shows images
                if i == 0 or i % 10 == 0:
                    clear_console()
