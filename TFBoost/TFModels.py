...
class TFModels():
    ...
    def index_buffer_data(self,value): self.index_buffer_data = value

    @property
    def labels_processed(self): return self._labels_processed

    @labels_processed.setter
    def labels_processed(self,value): self.labels_processed = value

    @property
    def inputs_processed(self): return self._inputs_processed

    @inputs_processed.setter
    def inputs_processed(self,value): self.inputs_processed = value
    ...
    @property
    def num_trains_count(self): return self._num_trains_acum

    @num_trains_count.setter
    def num_trains_count(self,value): self.num_trains_acum = value
    ...
    def convolution_model_image(self):
        ...
        x_reshape = tf.reshape(x, [-1, x1_rows_number, x1_column_number,
                                   1])  # Reshape x placeholder into a specific tensor
        # TODO Define shape and stddev in methods
        # First Convolutional Layer
        convolution_1 = tf.layers.conv2d(
            inputs=x_reshape,
        ...
        for epoch in range(num_epoch):
            for i in range(trains):
                ...
                if i == 0 or i % 10 == 0:
                    ...
                    pt('Seconds', (time.time() - start_time))
                    pt('Time', str(time.strftime("%Hh%Mm%Ss", time.gmtime((time.time() - start_time)))))
                    pt('TRAIN NUMBER: ' + str(num_trains_acum + 1) + ' | Percent Epoch ' + str(
                        epoch + 1) + ": " + percent_avance + '%')
                    pt('train_accuracy', train_accuracy)
                    pt('crossEntropyTrain', crossEntropyTrain)
                    pt('test_accuracy', test_accuracy)
                num_trains_acum += 1
